...
CONSECUTIVE_FRAMES_REQUIRED = 2  # Consecutive frames required inside ROI
...
class BasketballCounter:
    ...
    def get_status_text(self):
        ...
        if self.exam_state == ExamState.IDLE:
            status_lines.append("Status: Waiting Start")
            status_lines.append("Press 'Q' to start")
        elif self.exam_state == ExamState.RUNNING:
            status_lines.append(f"Exam in progress")
        ...
        while not exit_signal:
            if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
                ...
                if image_net is not None and image_net.size > 0:
                    image_bgr = cv2.cvtColor(image_net, cv2.COLOR_BGRA2BGR)
                    try:
                        if image_queue.full():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='/usr/local/zed/baketball-10-10.pt', help='model.pt path(s)')
